...
class CrossCompressUnit(Layer):
    def __init__(self, dim, batch_size, name=None):
        ...
        with tf.variable_scope(self.name):
            ...
            self.W = tf.get_variable(name='W', shape=(batch_size, dim), initializer=tf.zeros_initializer())
    ...
    def _call(self, inputs):
        ...
        v1 = tf.multiply(v, self.W)
        v = tf.expand_dims(v1, dim=2)
